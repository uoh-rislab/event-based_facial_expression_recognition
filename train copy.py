...
def main():
    # Configuración
    data_dir = 'output/e-ck+_frames_process_30fps/'
    #data_dir = 'output/ck+_frames_process_30fps/cropped_frames/'  # carpeta que contiene Train_Set y Test_Set

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = f"results/vit_e-ckplus_{timestamp}"
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == num_epochs - 1 or epoch % 5 == 0:
            ...
            inputs_vis = inputs_batch.cpu() * 0.5 + 0.5
            grid = utils.make_grid(inputs_vis[:4])
            writer.add_image(f'Predictions/Epoch_{epoch+1}', grid, epoch)
            utils.save_image(inputs_vis[:4], os.path.join(output_dir, f'example_predictions_epoch_{epoch+1}.png'))
            '''
