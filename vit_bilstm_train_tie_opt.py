...
# ===========================
class ViT_BiLSTM_Classifier(nn.Module):
    def __init__(self, embed_dim=768, hidden_dim=256, num_classes=7, num_layers=1): #hidden_dim=512
        super(ViT_BiLSTM_Classifier, self).__init__()
        ...
        self.lstm = nn.LSTM(
            ...
            batch_first=True,
            bidirectional=True,  # ✅ Cambiar para usar BiLSTM
            dropout=0.5  # Añadir dropout para evitar overfitting
        )

        
        # Capa de clasificación final
        self.fc = nn.Linear(hidden_dim * 2, num_classes)  # ✅ Multiplicar por 2
    ...
    def forward(self, images):
        ...
        out = torch.mean(lstm_out, dim=1)  # [batch_size, hidden_dim * 2]
...
for tie in ties:
    ...
    output_dir = f"results/tie-vit-bilstm/{timestamp}_vit_bilstm_e-ckplus_{fps}fps_{tie}"
    ...
    # ===========================
    model = ViT_BiLSTM_Classifier(num_classes=num_classes)
