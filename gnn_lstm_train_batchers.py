...
class DeepGNN_LSTM(torch.nn.Module):
    ...
    def forward(self, sequence):
        ...
        for data in sequence:
            ...
            x = global_mean_pool(x, batch)  # [batch_size, feature_dim]
            batch_outputs.append(x)

        # Stack: [seq_len, batch_size, feature_dim]
        x_seq = torch.stack(batch_outputs, dim=0)

        # Permute: [batch_size, seq_len, feature_dim]
...
for epoch in range(1, 151):
    ...
    for sequences, labels in tqdm(train_loader, desc=f"Epoch {epoch}"):
        ...
        for t in range(seq_len):
            graphs_at_t = [sequences[b][t].to(device) for b in range(len(sequences))]
            reorganized.append(Batch.from_data_list(graphs_at_t))

        labels = labels.to(device)

        # Verificar coherencia de batch size
        actual_batch_size = reorganized[0].num_graphs
        if actual_batch_size != labels.size(0):
            labels = labels[:actual_batch_size]

        out = model(reorganized)
        ...
        del loss, out, preds, labels, sequences
